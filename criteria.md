...
> 这一节对应 spec §14 的方案,**尚未实施**,条目先立在这里作为实施时的清单。

P0 —— 工具结果可见:
- [ ] 每个工具调用下面缩进显示结果摘要,默认最多 6 行,超出显示 `… +N lines`
- [ ] 给人的摘要和给模型的 `tool_result` 是两条独立的路径,截断摘要不影响模型拿到的完整内容
- [ ] `bash` exit code 非 0 时标红并显示 stderr,一眼能和成功区分开
- [ ] `edit_file`/`write_file` 显示 diff(`+` 绿 `-` 红),不是只打一行 `✎ edit path`
- [ ] `read_file` 只显示行数,不把文件内容重复打到终端

P1 —— 颜色与层次:
- [ ] 语义色板生效:用户输入/助手文本/工具调用/结果/错误/完成/元信息 各有稳定的样式
- [ ] `NO_COLOR` 设置后完全无色(不管值是什么);`TERM=dumb` 同样无色
- [ ] `FORCE_COLOR` 能在非 TTY 下强制上色
- [ ] **回归锁**:非 TTY 且未设 `FORCE_COLOR` 时,输出里一个转义序列都没有(管道/CI 场景)
- [ ] 上色后 §3.2 的 frame 擦除行数仍然正确(宽度计算先剥 ANSI)

P2 —— 状态指示:
- [ ] 执行中显示 spinner + 已用秒数,画在输入行上方,不干扰输入
- [ ] "在等模型"和"在跑工具"视觉上可区分
- [ ] 非 TTY 时不打 spinner,日志里不出现刷屏

P3 —— markdown:
- [ ] 粗体/行内代码/标题/列表/引用 按行渲染
- [ ] **回归锁**:半行仍然立刻流式输出(§3.2 优先),换行到达后再重绘该行的 markdown
- [ ] 跨 chunk 的标记(`**bold**` 的两个星号分在两次 delta)不会渲染错乱
