...
- [ ] **数据丢失回归锁**:一次粘贴 5 行,5 行全部进入同一条消息,一行都不丢(§15.1)
- [ ] 括号粘贴模式下,粘贴内容含换行也只产生一条消息,不拆成多个 turn
- [ ] 终端不支持括号粘贴时退化成逐行,但空闲期到达的行进队列而不是被丢弃
- [ ] `\` 结尾续行:不提交,提示符变成续行标记(§15.2)
- [ ] `/help` 列出全部命令与快捷键(§15.3)
- [ ] `/sessions` `/resume` `/new` `/compact` `/context` `/model` 各自生效,且不进 `session.messages`
- [ ] 未知的 `/xxx` 给出提示并列出可用命令,不发给模型
- [ ] Tab 补全文件路径,补全项不越出项目目录(§15.4/§6)
- [ ] `@path` 把文件内容随消息送出,终端上折叠成一行;越界路径被拒绝
- [ ] 输入历史落盘到 `<project>/.tcode/history`,重启后上翻仍在(§15.5)
- [ ] `tcode --resume` 不带 id 时列出并交互选择(§15.6)
- [ ] `tcode -p "<任务>"` 一次性执行后退出
- [ ] turn 结束时汇总本轮改动的文件
