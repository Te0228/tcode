...
- 执行前默认打印命令并等待用户确认(y/n,回车默认确认),除非启动时带 `--full-auto`。确认逻辑是独立的 `approval.ts`(见 7 节),不写死在工具实现里,方便以后升级成分级 policy。
- 实际执行通过一层 `executor.run(command, opts)` 封装(v1 是 `child_process.execSync`/`spawnSync` 的薄封装),不在工具里直接调用子进程 API——以后要接沙箱执行器(seatbelt/landlock/容器)时只替换这一层。
