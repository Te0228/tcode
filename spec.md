...
16 节把界面从"朴素"做到了"能看",但止步于此:两个一模一样的框、全部顶格、只有前景色。**问题不在细节而在结构**,所以这一节重做版式,并把规则固定成一套系统,而不是逐处调整。

### 17.1 三条结构决策

**1. 边杆分轨道,取消普通框。**

每一轮对话由一根左侧竖条分组:轮次标题用实心条 `▌`(强调色),该轮的正文用细条 `│`(暗色)。眼睛有了一条可循的轨道,一屏里能一眼看出有几轮对话、每轮从哪开始。

**框只留给模态**(确认框、选择器)。这是层次规则:**出现框 = 现在需要你处理**。之前欢迎横幅和输入框用一样的框,等于告诉用户"这两件事同等重要",而它们一个是装饰、一个是常驻输入区。

**2. 时间线。**

之前完全看不出"现在是第几轮""这轮跑了多久"。每轮标题带**轮次序号**和**时间**:
...
序号回答"我在哪",时间回答"什么时候、花了多久"。两者都右对齐,不占左侧轨道。

**3. 背景微染。**

之前只用前景色,所以画面永远是"彩色的字",没有块面。加三处背景:

- diff 的增/删行(绿底/红底),这是它看起来像编辑器而不是日志的关键
- 状态栏(整行反白底),让它读起来是一条"栏"而不是一行漂着的文字
- 选择器的当前项

**只在 truecolor 下上背景色。** 16 色的背景色是纯色块,在用户自定义主题下经常和前景撞成不可读;宁可退化成纯前景色。
...
| 角色 | 用途 |
|---|---|
| **主色(紫)** | 品牌:边杆、提示符、logo、轮次标题 |
| **副色(青)** | 交互:选中项、可操作元素、代码 |

- 语义色(错误红、新增绿、删除红、警告黄)**不参与品牌**,永远是它们自己。
- 灰阶三档:主要 / 次要 / 极弱。再多分不出来。
- **单强调色是"平庸"的主要来源**:整屏只有一个彩色,画面没有对比,读起来像单色终端加了点缀。两色分工明确既有对比又不花。

### 17.3 版式

```
▌ tcode  deepseek-chat                                ~/project

▌ 1 · you                                                14:32
│ Create auth.ts with getToken(req)

▌ 1 · tcode                                              14:32
│ I'll create the file, then fix the error handling.
│
│ ✎ auth.ts                                              +3 −0
│   1 │ export function getToken(req) {          ← 绿底
│   2 │   return req.headers.token ?? null       ← 绿底
│   3 │ }                                        ← 绿底
│
│ $ npm test                                             exit 0
│   12 passing
│
│ ✓ created auth.ts and verified                    14:32 · 18s

▌ › _
  deepseek-chat  4.2k/65.5k                    ⏎ send  esc stop
```

- **头部一行**,不是框。它说的话状态栏已经说了大半,占四行是浪费。
- **轨道宽 2 列**(条 + 空格),全局统一。工具行、结果、diff 全部在轨道右侧,缩进层级由轨道承担,不再各自缩进。
- **输入行是一根强调色边杆**,不是框。框会让它和模态撞车;边杆让它和上面的对话属于同一套语言。
- **状态栏整行铺底**,常驻。
- **轮与轮之间空一行**,轮内不空。节奏由留白承担。

### 17.4 交互状态

- **空态**:第一次进入时,轨道下方给一行提示(`try: fix the failing test` / `/help`),不是一片空白。
- **运行中**:轮次标题右侧的时间变成 spinner + 已用秒数,结束后定格成总耗时。用户始终知道"这轮跑了多久"。
- **中断**:该轮标题右侧标 `interrupted`,轨道保留——中断的一轮仍然是一轮,不该在时间线上消失。

### 17.5 硬约束(不变)

- 不进 alternate screen(见 16.1)。
- 非 TTY 全部退化成纯文本,不发任何转义序列。
- 一切宽度按显示宽度算;**带背景色的行必须截到正好宽度**,否则铺底会折行,整块背景错位。
- 无色时边杆退化成 ASCII(`|`),版式结构保留——结构本身不依赖颜色。

### 17.5b 命令菜单

斜杠命令做完之后,用户的反馈是"一个都不支持"。实测它们全部正常工作——**问题是敲下 `/` 之后屏幕没有任何反应**,于是用户合理地推断这个功能不存在。

这和 15.5 那次 `--continue` 是同一类错误:**功能在、发现不了,等于没有**。而且这次更隐蔽,因为 `/help` 本身就是为了解决发现性的,可它自己也得先被发现。

- 输入以 `/` 开头且还没有空格时,**输入行上方浮出命令菜单**,随着继续输入实时过滤。
- `↑`/`↓` 在菜单里选(菜单打开时接管这两个键,关闭后仍归历史),`Tab` 补全到输入行,`⏎` 直接执行选中项。
- 每项右侧带一句说明,和 `/help` 是同一份数据——**菜单和帮助不允许各写一份**,否则一定会有一边过期。
- 没有匹配项时菜单消失,输入照常发给模型(`/usr/local/bin` 这种要能正常说出来)。

### 17.5c 命令要覆盖它能做的每件事

第一版只有 8 个命令,是照着"先做几个最常用的"来的。这个取法不对:**命令集应该由这个工具实际具备的能力决定**,凡是它能做而只能靠重启进程或改环境变量才能触达的,都该有一条命令。照抄别人的清单同样不对——那会带进一堆 tcode 根本没有的概念。

按能力对照,完整集合:

| 命令 | 对应的能力 |
|---|---|
| `/help` | 命令与快捷键(它自己也得先被发现,见 17.5b) |
| `/status` | provider、model、root、session、运行时长、memory 层、审批模式,一屏说清 |
| `/model [name]` | 8 节的多 provider。**必须能真的切换**,不能只显示——否则换个模型仍然要退出改环境变量 |
| `/context` | 3.1 的 token 预算明细 |
| `/compact` | 3.1 的压缩,手动触发 |
| `/clear` | 清屏。不清历史——scrollback 是审计记录(16.1),清的是当前视野 |
| `/new` `/sessions` `/resume` | 4 节的会话管理 |
| `/export [file]` | 把当前会话导出成 markdown。会话是 JSON,人要拿去贴到别处时需要可读格式 |
| `/diff` | 5.3/5.4 改了哪些文件。在 git 仓库里就是 `git diff`,否则列出本次改动的文件 |
| `/undo` | **配套 `/diff` 的另一半**:它会改你的文件,而此前没有任何回退手段。范围是最近一轮 |
| `/memory` | 9 节的分层记忆:哪些层加载了、各自多少 token、内容是什么 |
| `/init` | 9 节的项目记忆冷启动:扫一遍项目生成初版 `AGENTS.md` |
| `/tools` | 5 节的工具集。子 agent 的工具集是裁剪过的(5.6),用户无从知道当前这个 agent 手里有什么 |
| `/approvals` | 5.1 的审批策略:当前是什么模式、本会话放行过哪些命令 |
| `/view` | 13.4 的 trace viewer,不必退出去敲 `tcode --view` |
| `/retry` | 重发上一条用户消息。模型跑偏时最常用的动作,此前要手动复制粘贴 |
| `/exit` `/quit` | 退出 |

**`/undo` 的实现约束**:回退需要改动前的内容,而这只有工具自己知道。所以由工具在 `ToolOutcome` 里回报"这个文件改动前是什么"(不存在则为 `null`),`runTurn` 收集成本轮的快照交给 REPL。**只保留最近一轮**——多级撤销要引入一套历史栈,而它和 git 的职责重叠,不值得在这里重造。

**不做的**:`/login`(key 在配置文件里,见 8.2)、`/cost`(没有计费信息)、`/vim`(输入层刚自己写完,再叠一套模式编辑是另一个量级的工程)、`/mcp`(还没有 MCP)。

### 17.6 谁负责排版

17 节做到一半时暴露出一个结构问题:**排版逻辑长在了 `agent.ts` 里**——它拼颜色、算宽度、决定缩进,再把成品字符串交给 REPL 打印。于是每加一条版式规则(边杆前缀、行宽扣掉轨道、轮次分组),都要回到 agent 里改一遍,而且总有漏网的路径(流式半行就漏了)。

这和 2 节"`index.ts` 只做展示层,不含 agent 逻辑"正好反过来了。

**改成事件流**:

- `runTurn` 不再接收 `log` / `writeText` / `palette` / 宽度,而是接收一个 `onEvent(event)`。
- agent 只报告**发生了什么**:助手文本片段、某个工具跑完了(带结果与元数据)、这一轮结束了、被中断了。它不知道颜色、不知道宽度、不知道有没有边杆。
- REPL 拿到事件后决定**怎么画**。边杆、轨道宽度、轮次标题、背景微染全部只在这一处实现,不可能漏。
- 子 agent 把事件透传上来并加一层 `depth`,由 REPL 决定嵌套怎么表现——而不是像现在这样自己拼一个 `│ ` 前缀。

**收益不只是整洁**:`agent.ts` 的测试从此不需要断言字符串长什么样,而 `ui/` 的测试可以喂一串事件、断言画面,两边都不再互相纠缠。
