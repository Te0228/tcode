...
- 正在执行的**单个工具不强杀**(比如一条跑了一半的 `npm test`)。`bash` 有自己的 `COMMAND_TIMEOUT_MS`,强杀子进程会留下不一致的中间状态,得不偿失。中断的语义是"这一轮别再往下走了",不是"立刻停止一切"。
- 中断后 `messages` 里会留下一条说明"用户中断了这一轮",让模型在下一轮知道上一轮没做完。
- **第二次 Ctrl+C 才退出进程**。第一次是中断当前 turn,第二次是真的想走。turn 没在跑时,Ctrl+C 直接退出。
- 不做 Esc 键中断:readline 下要拦 Esc 得接管 raw mode,会和正常输入(方向键、粘贴)打架,代价不值。

**执行期间的输入**:

- turn 执行期间 REPL 继续接受输入,输入的内容**进队列**,当前 turn 结束后作为下一轮的 user 输入自动发出。
- 排队时给出可见反馈(如 `⏎ queued`),否则用户不知道自己敲的东西有没有被吃掉。
- 不做"把排队的消息插进正在跑的 turn 中间"——那会破坏消息历史的结构。排队消息是**下一轮**的输入。
...
- 实际执行通过一层 `executor.run(command, opts)` 封装(`child_process.spawn` 的薄封装,**异步**),不在工具里直接调用子进程 API——以后要接沙箱执行器(seatbelt/landlock/容器)时只替换这一层。
  - **必须异步,不能用 `spawnSync`。** `spawnSync` 会阻塞整个 event loop,期间信号处理器无法运行——用户按下 Ctrl+C 后要等命令自己跑完(最长 `COMMAND_TIMEOUT_MS`)才会有任何反应,3.2 的中断形同虚设。这是实测踩出来的坑,不是理论问题。
